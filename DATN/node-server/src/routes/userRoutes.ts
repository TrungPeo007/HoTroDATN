...
router.get("/", async (req: Request<{}, {}, {}, GetAllUser>, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ thong_bao: "Lỗi máy chủ khi lấy danh sách người dùng" });
  }
});
router.post(
  "/",
  ...
  async (req: Request<{}, {}, CreateUser>, res: Response) => {
    ...
    try {
      const { tai_khoan, email, mat_khau, mat_khau_nhap_lai, ho_ten, vai_tro } =
        req.body;
      ...
      if (existing) {
        if (existing.tai_khoan === taiKhoanTrim) {
          throw {
            status: 409,
            thong_bao: "Tài khoản đã tồn tại, vui lòng Nhập tài khoản khác nhá",
          };
        }
        if (existing.email === emailTrim) {
          throw {
            status: 409,
            thong_bao: "Email đã tồn tại, vui lòng nhập email khác",
          };
        }
      }
      ...
      return res.status(200).json({
        thong_bao: ` Thêm thành công User có id là ${newUser.id} và role là ${role}`,
        success: true,
      });
    } catch (error) {
      await cleanUpfiles();
...
router.put<UserParams, {}, UpdateUser>(
  "/:id",
  uploadMiddleware,
  async (req, res) => {
    ...
    try {
      ...
      const { mat_khau, mat_khau_nhap_lai, ho_ten, dien_thoai, vai_tro, khoa } =
        req.body;
      const user = await User.findByPk(id);
      if (!user) {
      ...
      if (mat_khau) {
        ...
        if (
          ...
          throw {
            status: 400,
            thong_bao:
              "Mật  khẩu phải trên  8 ký tự, có 1 chữ in hoa và 1 ký tự đặc biệt",
          };
        }
        if (mat_khau != mat_khau_nhap_lai) {
          throw {
      ...
      if (Object.keys(allowedUpdate).length > 0) {
        ...
        return res.status(200).json({
          thong_bao: `Đã cập nhật user có ID là ${updateUser.id}`,
          success: true,
        });
      }
      const unUpdateUser = user.toJSON();
      return res.status(200).json({
        thong_bao: ` Không có cập nhật gì ở User có ID là ${unUpdateUser.id}`,
        success: true,
      });
    } catch (error) {
      await cleanUpfiles();
...
router.delete("/:id", async (req: Request<UserParams>, res: Response) => {
  try {
    ...
    if (!user) {
      throw {
        status: 404,
        thong_bao: "Người dùng không tồn tại nên không thể xóa",
      };
    }
    ...
    return res
      .status(200)
      .json({ thong_bao: "Đã xóa thành công user", success: true });
  } catch (error) {
    const err = error as CustomError;
    const status = err.status || 500;
...
// Route PUT /:id/toggle-lock - Toggle lock user (không cần xác thực admin)
router.put<UserParams>("/:id/toggle-lock", async (req, res) => {
  try {
    const { id } = req.params;
    ...
    if (isNaN(Number(id))) {
      return res.status(400).json({
        thong_bao: "ID người dùng không hợp lệ",
        success: false,
      });
    }
    ...
    if (!user) {
      return res.status(404).json({
        thong_bao: "Không tìm thấy user để cập nhật",
        success: false,
      });
    }

    // Toggle trạng thái khóa
    const currentStatus = user.khoa;
    ...
    // Tăng token_version để vô hiệu hóa token cũ (nếu có)
    ...
    return res.status(200).json({
      thong_bao: `Đã ${
        newStatus ? "khóa" : "mở khóa"
      } tài khoản người dùng ID ${user.id}`,
      success: true,
      result: {
        id: user.id,
        khoa: user.khoa,
    ...
    return res.status(500).json({
      thong_bao: "Lỗi máy chủ khi thay đổi trạng thái tài khoản",
      success: false,
    });
  }
});
