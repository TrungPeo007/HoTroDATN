...
import fs from 'fs';
import { createSanPhamInput, createSanPhamSchema, sanPhamIdSchema, updateSanPhamInPut, updateSanPhamSchema } from '../schema/sanpham.schema';
...
import { DanhMucTin, DM_San_Pham, IMG_SanPham, SanPham, SanPhamBienThe, ThuocTinh, ThuocTinhSP, ThuongHieu } from '../models';
import { Op } from 'sequelize';
import { generateSku, generateSlug } from '../ultis/slugrename';
import { validateForeignKey } from '../ultis/validate';

import { AuthUser } from '../types/express';
import { createBienTheSp, createThuocTinhSp, GetALLSanPHam, ImgBienThe, ThuocTinhMap } from '../types/sanpham';
...
router.post<{},{}, createSanPhamInput>('/',uploadMiddleware, validate(createSanPhamSchema),async(req,res)=>{
	...
	try {
	   ...
	   if(generatedFiles.length > 0){
		await Promise.all(generatedFiles.map(filePath=>{
...
router.get<updateSanPhamInPut['params']>('/:id',validate(sanPhamIdSchema),async(req, res)=>{
	try {
		...
		const sanPham = await SanPham.findByPk(id,
			{
				attributes: ['id','ten_sp','code','slug','img','gia','sale','so_luong','da_ban','luot_xem','xuat_xu','dvctn','dvt','noi_bat','mo_ta','an_hien','id_dm','id_th','createdAt'],
					include: [{
						model: SanPhamBienThe,
						as: 'san_pham_bien_the',
						attributes: ['id','id_sp','code','ten_bien_the','gia','so_luong','img','createdAt'],
