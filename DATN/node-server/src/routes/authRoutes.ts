...
import { CustomJwtPayload, DangKyBody, dataToSendLogin, DoiMatKhauBody, GuiLaiXacThucDKBody, mailOptions, QuenMatKhauBody, RefreshJwtPayload, SafeUserData, XacThucDangKy } from '../types/auth';
...
import { checkAuth, handleFacebookAuth, handleGoogleAuth } from '../middleware/auth';
...
router.post('/api/quen-pass',resendLimiter,async(req: Request<{}, {}, GuiLaiXacThucDKBody>, res: Response)=>{
	try {
		...
		if(user.provider !== AUTH_PROVIDER.LOCAL || !user.mat_khau){
			console.warn("Không thể đổi  mật khẩu trong cục bộ vì tài khoản đã đăng ký bằng gmail hoặc google")
...
router.post('/api/xac-thuc-otp-doi-pass',async(req:Request<{}, {}, QuenMatKhauBody >, res: Response)=>{
	try {
		...
		if(user.otp !== otp){
			throw {status: 404, thong_bao: "OTP không đúng vui lòng kiểm tra lại"};
		}
		if(!user.mat_khau){
...
router.get('/api/auth/google/callback', handleGoogleAuth,async(req: Request, res: Response)=>{
	try {
		...
		const safeUserData: SafeUserData = {
			ho_ten: user.ho_ten, tai_khoan: user.tai_khoan, email: user.email, vai_tro:user.vai_tro
		}
		const token = jwt.sign(
			TokenPayLoad,
			process.env.JWT_SECRET ||'your secret pass',
...
router.get('/api/auth/facebook/callback',handleFacebookAuth,(req: Request, res: Response)=>{
	try {
		const user = req.user as  User;
		const TokenPayLoad : CustomJwtPayload = {
			id: user.id, tai_khoan: user.tai_khoan, vai_tro: user.vai_tro, ho_ten: user.ho_ten, token_version: user.token_version
...
// router.post('/api/hinhtest', uploadMiddleware,async (req: Request, res: Response) => {
		
// 		try {

// 			const filesInfo = req.files ? Object.values(req.files).flat().map(f => f.filename) : [];
			
// 			res.status(200).json({ 
// 				status: 'success',
// 				message: 'Upload và kiểm tra thành công!', 
...
// 			res.status(500).json({ 
// 				status: 'error',
// 				message:  'Lỗi xử lý nghiệp vụ máy chủ.' 
// 			});
// 		}
// 	});
